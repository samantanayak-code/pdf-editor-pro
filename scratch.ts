...
import { TextToken, TextLine, StructuredDocument } from './types';
import { extractStructuredDocument, detectPageRegions } from './textExtraction'; // Adjust imports if necessary based on converter.ts
import { inferCellType } from './cellTypes'; // assuming this exists based on the original code
...
export function extractVectorBoundaries(
...
export function extractTablesForExcel(
  allLines: TextLine[],
  pageWidth: number,
  pageRects?: Array<{ x: number; y: number; width: number; height: number }>
): ExcelTable[] {
  ...
  if (hasVectorCols && hasVectorRows) {
    ...
    const rowHeightsPts = Array(numRows).fill(14); // Default 14pt
    ...
    for (const cell of cells) {
       ...
       // distribute height requirement across spanned rows
...
export function styleExcelSheet(
  ws: XLSX.WorkSheet,
  table: ExcelTable,
  pageWidth: number
): void {
  ...
  table.rowHeightsPts.forEach((hpt, r) => {
     (ws['!rows'] as any[])[r] = { hpt: hpt + 4 };
  });

  const merges: XLSX.Range[] = [];

  // find max R and max C for looping formatting
  ...
  // Apply rich cell styling and detect merges
  for (const cell of table.cells) {
    ...
    if (/^[0-9,]+(\.[0-9]+)?$/.test(cellV.trim())) {
        t = 'n';
        cellV = cellV.replace(/,/g, '') as any;
    }
    ws[addr].v = cellV;
    ws[addr].t = t;

    // style all cells in span to fix borders
    for (let rr = 0; rr < cell.rowSpan; rr++) {
       for (let cc = 0; cc < cell.colSpan; cc++) {
          ...
          // Outer borders for merged areas
